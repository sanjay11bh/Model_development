...
class Models:
        ...
        if is_classification:
            ...
            if len(np.unique(y_test)) == 2:
                try:
                    ...
                    print("ROC AUC plot skipped — model does not support `predict_proba`.")
            ...
            plt.figure(figsize=(8, 6))
            sns.scatterplot(x=y_test, y=self.predictions)
    ...
    def fit_predict_evaluate(self, model, X_train, X_test, y_train, y_test, is_classification=True):
        ...
        if is_classification:
            ...
            # Combined plot: Confusion Matrix + ROC curve if binary classification
            ...
            # Combined plot: Actual vs Predicted scatter + Residuals histogram
            fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    ...
    def Logistic_regression(self, X_train, X_test, y_train, y_test):
        ...
        solver = self.get_param("Solver (lbfgs/liblinear/saga/newton-cg)", "lbfgs", str)
        max_iter = self.get_param("Max iterations", 1000, int)
...
class optuna_Model:
    ...
    def optuna_visualization(self ):
        if not hasattr(self, "study"):
            print("No study available. Run a model training method with Optuna first.")
            return
...
class AutoModelSelector:
    ...
    def run_regression(self, X_train, y_train, X_test, y_test):
